...
public class scTest8 : MonoBehaviour, IVirtualButtonEventHandler {
	...
	public void OnButtonPressed(VirtualButtonAbstractBehaviour vb)
	{
		...
		for(int i=1;i<4;i++)
		{
			for(int j=-1;j<2;j++)
			{
				if(y!=.24f || z!=0)
				{
					GameObject obj=FindAt(new Vector3((float)(x),(float)(y*i),(float)(z*j)));
					//Debug.Log(obj.name);
